...
| Evidence, records, contracts | `specs/003-ci-oidc-deploy/` |
| Entra and Azure objects | no path — the task records its outcome in `specs/003-ci-oidc-deploy/evidence/` |

**Before any `az` or `gh` command**: `export PATH="/usr/local/bin:$PATH"`.

**Redaction rule, binding on every evidence file.** `origin` is public. R10 chose
to store the tenant, subscription and client identifiers as secrets rather than
variables precisely so they are not published. Evidence files therefore elide
them the way [research.md](research.md) does — `5900fbc9-…` — and never contain a
full identifier. A captured run log is copied verbatim *except* for that
substitution, which is noted inline where it is applied.
...
- [ ] T001 Capture the six-resource baseline to `specs/003-ci-oidc-deploy/evidence/inventory-before.json` using the `az resource list` command in [quickstart.md](quickstart.md). This must be taken before any other task; SC-002 is a diff against it.
- [ ] T002 [P] Register the `ai300-github-deploy` application and its service principal with `az ad app create` / `az ad sp create`, adding **no** password and **no** certificate at any point (FR-002), and record the client id and the service principal object id — redacted — in `specs/003-ci-oidc-deploy/evidence/identity-ids.md`.
- [ ] T003 [P] Create the empty probe resource group `rg-ai300-probe` in `northeurope` and note its creation in `specs/003-ci-oidc-deploy/evidence/identity-ids.md`. It holds nothing and exists only so probe P2 has a named target (FR-017b).
- [ ] T004 Enumerate the application's credentials with both `az ad app credential list` forms and append the two zero counts to `specs/003-ci-oidc-deploy/evidence/no-credentials.md` as the *before* half of SC-006. Depends on T002.
- [ ] T005 Create the GitHub environment `azure-deploy` with the author as required reviewer, **Prevent self-review off** (R5), deployment branches limited to `main`, and record the resulting settings in `specs/003-ci-oidc-deploy/evidence/environment-config.md`.
- [ ] T006 Store `AZURE_CLIENT_ID`, `AZURE_TENANT_ID` and `AZURE_SUBSCRIPTION_ID` as repository secrets via `gh secret set`, and record in `specs/003-ci-oidc-deploy/evidence/environment-config.md` that these are identifiers, not credentials (R10) — the claim SC-004's third refusal exists to settle. Depends on T002.
...
- [ ] T007 Add `.github/workflows/oidc-claims-probe.yml` — a temporary workflow, `workflow_dispatch` only, `id-token: write`, **no `environment:`** — that requests a token and prints only its `sub`, `aud` and `iss` claims. **The token itself is never printed**, only the three claims decoded out of it (R3).
- [ ] T008 Run `oidc-claims-probe.yml` and record the observed subject verbatim in `specs/003-ci-oidc-deploy/evidence/oidc-subject.md`, alongside the format R3 predicted. If the two differ, the observed value wins and R3 is annotated with what was actually issued — observation over documentation, including over R3's own.
- [ ] T009 Create the federated identity credential on `ai300-github-deploy` from the **observed** subject in T008, issuer `https://token.actions.githubusercontent.com`, audience `api://AzureADTokenExchange`, and record the created object — subject redacted only where it carries a numeric owner id — in `specs/003-ci-oidc-deploy/evidence/identity-ids.md`. Depends on T008.
...
- [ ] T011 Validate `infra/ci-identity.bicep` with `az bicep build` and report the result, warnings included, before proposing it for commit (Principle V). Depends on T010.
- [ ] T012 [P] Extend `.github/workflows/bicep-validate.yml` so the build step globs every `*.bicep` under `infra/` instead of naming `main.bicep`. No permission is added and no token is requested — a fork pull request must keep running it exactly as today (FR-013, R6).
- [ ] T013 Deploy `infra/ci-identity.bicep` as the author with `az deployment group create`, passing the service principal object id, and record the deployment name in `specs/003-ci-oidc-deploy/evidence/identity-ids.md`. Depends on T011.
...
- [ ] T014 [US1] Write `.github/workflows/infra-deploy.yml` with the `deploy` job only: triggers `push` to `main` on `infra/**` plus `workflow_dispatch`, `environment: azure-deploy`, permissions `id-token: write` and `contents: read` and nothing else, `concurrency` with `cancel-in-progress: false`, actions pinned to the SHAs in [contracts/workflow-contract.md](contracts/workflow-contract.md), and a real `az deployment group create -n ai300-ci-<run_id>` — no preview, no what-if (FR-010).
...
- [ ] T016 [US1] Before adding each operation in T015, confirm the failure is an authorization refusal that names it, and not a queued deployment, a transient error, or an unregistered provider (FR-017 applies to discovery as much as to the probes). Record any rejected candidate, with the reason, in `specs/003-ci-oidc-deploy/evidence/discovery-log.md`.
- [ ] T017 [US1] Record the green run id and the `Succeeded` deployment record in `specs/003-ci-oidc-deploy/evidence/deployment-record.md`, noting explicitly that the failed records above it are discovery evidence and not counted against SC-001.
- [ ] T018 [US1] Capture `specs/003-ci-oidc-deploy/evidence/inventory-after.json` and diff it against `inventory-before.json`; record the empty diff in `specs/003-ci-oidc-deploy/evidence/deployment-record.md` (SC-002, FR-011).
- [ ] T019 [US1] Re-run the workflow unchanged, confirm it succeeds again and that a second inventory capture still diffs clean, and record both run ids in `specs/003-ci-oidc-deploy/evidence/deployment-record.md` (FR-012).
...
- [ ] T020 [US2] Add the `boundary` job to `.github/workflows/infra-deploy.yml`: runs after `deploy`, same environment, same principal, executing probes P1–P4 exactly as written in [contracts/boundary-probes.md](contracts/boundary-probes.md).
- [ ] T021 [US2] Implement the assertion rule in that job: a probe passes only when the command exits non-zero **and** its stderr names an authorization refusal. A probe that succeeds fails the run; a probe that fails for any other reason also fails the run (FR-017a). This is what makes SC-003 a standing test rather than a one-day capture.
- [ ] T022 [US2] Run the workflow and copy the four commands and their errors verbatim into `specs/003-ci-oidc-deploy/evidence/refusals-authorization.md`, one section per probe, with the run id — the exact command and the exact error, not a description of why the configuration forbids it (FR-016).
...
- [ ] T024 [US2] If any probe unexpectedly succeeded, delete what it created immediately, record the fact as a defect in `specs/003-ci-oidc-deploy/evidence/refusals-authorization.md`, and narrow `infra/ci-identity.bicep` before continuing. A widened boundary is the one failure mode of this feature that is not normal.
...
- [ ] T025 [US3] Re-run `oidc-claims-probe.yml` on `main` **now that the federated credential exists**, with an `azure/login` step and no `environment:`, and capture the refusal in `specs/003-ci-oidc-deploy/evidence/refusals-authentication.md` as A1. This is the sharp version of SC-004's first refusal: correct repository, correct branch, gate not passed. The setup-time failure from T008 is the weak version — it proves only that nothing was configured yet — and is recorded alongside it as such. The run needs no gate approval, precisely because it does not enter the environment.
- [ ] T026 [US3] Run the same probe from the feature branch rather than `main`, so the subject names a `ref` context that satisfies the trust condition not at all, and capture the refusal as A2 in `specs/003-ci-oidc-deploy/evidence/refusals-authentication.md`.
...
- [ ] T028 [US3] Confirm in `specs/003-ci-oidc-deploy/evidence/refusals-authentication.md` that all three errors are authentication errors — `AADSTS…`, not `AuthorizationFailed` — and say so explicitly, since the distinction is the entire content of the criterion.
...
- [ ] T029 [US4] Open a pull request that modifies something under `infra/`, confirm `bicep-validate.yml` runs green — including on `ci-identity.bicep`, which T012 brought into its build scope — and record the run id in `specs/003-ci-oidc-deploy/evidence/pull-request-runs.md`.
- [ ] T030 [US4] Confirm with `gh run list --workflow infra-deploy.yml --event pull_request` that the deploying workflow has **no** run for that event, and record the empty result in `specs/003-ci-oidc-deploy/evidence/pull-request-runs.md` (SC-005, FR-014).
...
- [ ] T032 [US4] Record in `specs/003-ci-oidc-deploy/evidence/pull-request-runs.md` the limit this evidence has: no genuine fork pull request was authored, so SC-005 is settled by what the repository runs plus the three independent barriers in R6 — not by a fork actually trying. The checklist already carries this; it belongs with the evidence too.
...
- [ ] T035 [US5] Withdraw the role assignment with `az role assignment delete`, run the workflow, approve the gate, and record the authorization failure and its run id in `specs/003-ci-oidc-deploy/evidence/necessity.md`. A run that still succeeds means something other than this grant is authorizing it — the 002 outcome, and a failed criterion.
- [ ] T036 [US5] Restore the assignment by redeploying `infra/ci-identity.bicep`, run the workflow again, and record the succeeding run id in `specs/003-ci-oidc-deploy/evidence/necessity.md`. The final state of the environment must be the working one (spec edge case).
- [ ] T037 [US5] In `specs/003-ci-oidc-deploy/evidence/necessity.md`, tabulate each operation in the final role against the derivation line or verification run that accounts for it, and state the two counts SC-007 compares. Zero operations may survive unaccounted for.
...
- [ ] T038 Enumerate the application's credentials again with both `az ad app credential list` forms and append the two zero counts, with the date, to `specs/003-ci-oidc-deploy/evidence/no-credentials.md` as the *after* half of SC-006 — meaningful only because SC-001 has already passed.
- [ ] T039 [P] Run the cost report for the days spanning the feature and record it in `specs/003-ci-oidc-deploy/evidence/cost-report.md`, confirming no new meter and a total of `0.00` (SC-008, FR-019).
- [ ] T040 Delete `.github/workflows/oidc-claims-probe.yml`. It has served its two purposes — the subject in T008, the refusals in T025 and T026 — and a workflow that requests a token outside the gate should not outlive them.
- [ ] T041 Revise `infra/DEPLOY.md` with the identity runbook: the creation order from [data-model.md](data-model.md), the CI deployment path, the immutable-subject trap (R3), and **the stale-authority warning** — when `main.bicep` gains a resource type the next deployment *will* fail, and that is FR-006 working. The next person must find that written down rather than debug it.
- [ ] T042 Write the reversal into `infra/DEPLOY.md` as runnable commands — role assignment, role definition, federated credential, service principal, application, probe resource group, three secrets, environment — and confirm the count of removal commands matches the count of objects created (SC-009, FR-018). The reversal is recorded, not executed; the environment is left working.
- [ ] T043 Sweep every file under `specs/003-ci-oidc-deploy/evidence/` for full tenant, subscription, client or object identifiers and apply the redaction rule stated at the head of this file. `origin` is public, and R10's reasoning about not publishing identifiers applies to evidence as much as to repository variables.
- [ ] T044 Update `specs/003-ci-oidc-deploy/checklists/requirements.md`: close the two Deferred items (overlapping runs — settled by `cancel-in-progress: false`; where evidence is stored — settled by `evidence/`), and confirm the two recorded limits still read honestly.
...
- [ ] T046 Walk [quickstart.md](quickstart.md) end to end against the finished state and correct anything it now describes wrongly.
...
| 5 | `specs/003-ci-oidc-deploy/` — evidence and the closing record | T017–T019, T022–T023, T025–T028, T029–T032, T035–T037, T038–T039, T043–T044 |
